 ...
 * @property    {string[]}  dayHeaderNames                              The names to use for the day headers (defaults to '[ "Pon", "Wto", "Śro", "Czw", "Pią", "Sob", "Nie" ]').
 * @property    {string[]}  dayNames                                    The full names (defaults to '[ "Poniedziałek", "Wtorek", "Środa", "Czwartek", "Piątek", "Sobota", "Niedziela" ]').
 * @property    {string[]}  monthNames                                  The names to use for months (defaults to '[ "Styczeń", "Luty", "Kwiecień", "Marzeć", "Maj", "Czerwiec", "Lipiec", "Sierpień", "Wrzesień", "Październik", "Listopad", "Grudzień" ]').
...
function calendarJs( id, options, searchOptions, startDateTime ) {
   ...
   /* function showFullDayView( date, fromOpen ) {
    ...
    function eventDialogEvent_OK() {
     confirmRequest(_element_EventEditorDialog_EventDetails.from, _element_EventEditorDialog_EventDetails.to);
    ...
    console.log(_element_EventEditorDialog_EventDetails);
...
window.location.reload();
    ...
    console.log(_element_EventEditorDialog_EventDetails);
      _element_EventEditorDialog_DisabledArea.style.display = "block";
        ...
        var onYesEvent = function() {
        	console.log('on yes event');
revoke( _element_EventEditorDialog_EventDetails.from, _element_EventEditorDialog_EventDetails.to)
    ...
    this.addEvent = function( event, updateEvents, triggerEvent, setLastUpdated ) {
		...
		//confirmRequest(event.from, event.to);
        ...
        //console.log('add this new event');
    ...
    function setTranslationStringOptions() {
        if ( !isDefinedString( _options.previousMonthTooltipText ) ) {
            _options.previousMonthTooltipText = "Poprzedni miesiąc";
        }

        if ( !isDefinedString( _options.nextMonthTooltipText ) ) {
            _options.nextMonthTooltipText = "Następny miesiąc";
        }

        if ( !isDefinedString( _options.previousDayTooltipText ) ) {
            _options.previousDayTooltipText = "Wczoraj";
        }

        if ( !isDefinedString( _options.nextDayTooltipText ) ) {
            _options.nextDayTooltipText = "Jutro";
        }

        if ( !isDefinedString( _options.previousWeekTooltipText ) ) {
            _options.previousWeekTooltipText = "Poprzedni tydzień";
        }

        if ( !isDefinedString( _options.nextWeekTooltipText ) ) {
            _options.nextWeekTooltipText = "Następny tydzień";
        }

        if ( !isDefinedString( _options.addEventTooltipText ) ) {
            _options.addEventTooltipText = "Dodaj wydarzenie";
        }

        if ( !isDefinedString( _options.closeTooltipText ) ) {
            _options.closeTooltipText = "Zamknij";
        }

        if ( !isDefinedString( _options.exportEventsTooltipText ) ) {
            _options.exportEventsTooltipText = "Wyeksportuj wydarzenia";
        }

        if ( !isDefinedString( _options.todayTooltipText ) ) {
            _options.todayTooltipText = "Dzisiaj";
        }
        ...
        if ( !isDefinedString( _options.titleText ) ) {
            _options.titleText = "Tytuł:";
        }

        if ( !isDefinedString( _options.descriptionText ) ) {
            _options.descriptionText = "Opis:";
        }

        if ( !isDefinedString( _options.locationText ) ) {
            _options.locationText = "Lokalizacja:";
        }
    ...
    function getStandardHolidays() {
        return [
            ...
            },
            {
                day: 25,
                month: 12,
                title: "I Dzień Bożego Narodzenia"
            },
            {
                day: 26,
                month: 12,
                title: "II Dzień Bożego Narodzenia"
            }
        ];
    }
...
function revoke( date_from, date_to) {
	console.log(date_from.getFullYear+"; "+date_to);
	var loggedUser = $('#loggedUser').val();
	var vacation = {
		date_from: date_from,
		date_to: date_to,
